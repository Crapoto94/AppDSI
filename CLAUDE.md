...
- **Agent presence indicator**: whenever a page displays an agent's name and/or email (requester, technician, beneficiary, etc.), render `<AgentPresenceBadge email={...} name={...} />` (`frontend/src/components/AgentPresenceBadge.tsx`) right next to it. It queries the RH Studio API (config in `hub.infra_apis`, key `rh_studio_presence`, proxied via `GET /api/infra/agents/presence`) — looking up by email first, falling back to name search if the email doesn't match — and shows one of four badges with an explanatory tooltip: ⭐ present, ❌ departed (with departure date if known), ⏳ not yet arrived (with expected arrival date if known), ❓ unknown to RH Studio. Results are cached in-memory per session (module-level cache in the component, same pattern as `UserHoverCard`). Already wired into `/certif` (beneficiaries) and `/tickets` (requesters, technicians); apply the same pattern when adding agent displays to other modules.
