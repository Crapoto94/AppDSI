...
async function runMigration() {
  try {
    ...
    const existing = await pool.query(
      `SELECT id FROM hub.tiles WHERE title = 'VibeCoding'`
    );
    ...
    if (existing.rows.length === 0) {
      ...
      const result = await pool.query(`
        ...
        VALUES (
          'VibeCoding',
          'Terminal',
          'Ressources et plan de la formation au développement assisté par IA',
          'active',
      ...
      // Add link to the VibeCoding resources page
      await pool.query(`
        INSERT INTO hub.tile_links (tile_id, label, url, is_internal)
        VALUES ($1, 'Accéder', '/vibecoding', 1)
      `, [tileId]);
      ...
      const linkExists = await pool.query(
        `SELECT id FROM hub.tile_links WHERE tile_id = $1 AND url = '/vibecoding'`,
        [existing.rows[0].id]
      );
