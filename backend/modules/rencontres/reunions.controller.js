...
// Construit le contenu HTML du compte rendu d'une réunion. Partagé entre la
// prévisualisation (validation avant envoi) et l'envoi par email. Renvoie null
// si la réunion est introuvable. `isEmpty` = aucun contenu exploitable.
async function buildCompteRendu(reunionId) {
    ...
    const participants = await pgDb.all('SELECT * FROM reunion_participants WHERE reunion_id=? ORDER BY nom', [reunionId]);

    // La rubrique « Demandes » n'a de sens que pour une réunion issue du module
    // de demandes budgétaires du DSI Hub — jamais pour le module Réunions générique.
    const includeDemandes = (reunion.source || '') === 'rencontres_budgetaires';
    const demandes = includeDemandes
        ? await pgDb.all('SELECT * FROM rencontres_budgetaires WHERE reunion_id=? ORDER BY direction, service, titre', [reunionId])
        : [];
    ...
    const convoques = participants.filter(p => p.statut_presence === 'info');
    const fmtPerson = (p) => `${p.prenom ? p.prenom + ' ' : ''}${p.nom}${p.service ? ' (' + p.service + ')' : ''}`;
    const listPills = (list, color, bg) => list.length > 0
        ? list.map(p => `<span style="display:inline-block;padding:2px 8px;background:${bg};color:${color};border-radius:4px;font-weight:600;margin:1px 2px">${fmtPerson(p)}</span>`).join(' ')
  ...
  <tr><td style="padding:10px 14px;background:#f8fafc;font-weight:700;color:#475569;border-bottom:1px solid #e2e8f0;word-wrap:break-word;word-break:break-word">👥 Présents (${presents.length})</td>
      <td style="padding:10px 14px;color:#1e293b;border-bottom:1px solid #e2e8f0;word-wrap:break-word;word-break:break-word">${listPills(presents, '#16a34a', '#dcfce7')}</td></tr>
  <tr><td style="padding:10px 14px;background:#f8fafc;font-weight:700;color:#475569;border-bottom:1px solid #e2e8f0;word-wrap:break-word;word-break:break-word">❌ Excusés (${excuses.length})</td>
      <td style="padding:10px 14px;color:#1e293b;border-bottom:1px solid #e2e8f0;word-wrap:break-word;word-break:break-word">${listPills(excuses, '#dc2626', '#fee2e2')}</td></tr>
  <tr><td style="padding:10px 14px;background:#f8fafc;font-weight:700;color:#475569;border-bottom:1px solid #e2e8f0;word-wrap:break-word;word-break:break-word">🔔 Conviés (${convoques.length})</td>
      <td style="padding:10px 14px;color:#1e293b;border-bottom:1px solid #e2e8f0;word-wrap:break-word;word-break:break-word">${listPills(convoques, '#4338ca', '#e0e7ff')}</td></tr>
</table>`;

    const deroulePlain = (reunion.description || '').replace(/<[^>]*>/g, '').trim();
...
<div style="margin-bottom:16px;padding:12px 16px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;font-size:13px">
  ...
  ${lienProjet.comite_nom ? `<br><strong style="color:#16a34a">Comité :</strong> <span style="color:#1e293b">${lienProjet.comite_nom}</span>` : ''}
    ...
    const demandesSection = includeDemandes ? `
...
<table style="width:100%;max-width:100%;table-layout:fixed;border-collapse:collapse;font-size:13px">
  ...
  <tbody>${demandesHtml}</tbody>
    ...
    const isEmpty = !deroulePlain && decisions.length === 0 && tasks.length === 0 && demandes.length === 0;
...
${demandesSection}`;
    ...
    const recipients = participants
        ...
        .map(p => ({ email: p.email, name: `${p.prenom ? p.prenom + ' ' : ''}${p.nom || ''}`.trim() || p.email, statut_presence: p.statut_presence }));

    const subject = `Compte rendu — ${reunion.titre}${dateStr ? ' du ' + dateStr : ''}${lienProjet?.projet_titre ? ' [' + lienProjet.projet_titre + ']' : ''}`;

    return { reunion, content, subject, recipients, isEmpty, dateStr };
}
...
module.exports = {
        ...
        try {
            ...
            if (Array.isArray(participants) && participants.length > 0) {
                for (const p of participants) {
                    await pgDb.run(
                        `INSERT INTO reunion_participants (reunion_id, nom, prenom, email, service, direction, type_presence, statut_presence, ad_username, commentaire, organisme, fonction) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                        [reunionId, p.nom, p.prenom || '', p.email || '', p.service || '', p.direction || '', p.type_presence || 'metier', p.statut_presence || 'present', p.ad_username || null, p.commentaire || null, p.organisme || null, p.fonction || null]
                    );
            ...
            if (!addedUsernames.has(username)) {
                const userInfo = await pgDb.get('SELECT displayName, email, service_code, service_complement FROM hub.users WHERE username=?', [username]);
                if (userInfo) {
                    ...
                    await pgDb.run(
                        `INSERT INTO reunion_participants (reunion_id, nom, prenom, email, service, type_presence, statut_presence, ad_username) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                        [reunionId, nom, prenom, userInfo.email || null, userInfo.service_complement || userInfo.service_code || null, 'metier', 'present', username]
                    );
                }
            }
            if (!creatorEmail) {
    ...
    getById: async (req, res) => {
        try {
            ...
            // Les demandes ne concernent que les réunions issues du module de
            // demandes budgétaires ; le module Réunions est générique.
            reunion.demandes = (reunion.source === 'rencontres_budgetaires')
                ? await pgDb.all('SELECT * FROM rencontres_budgetaires WHERE reunion_id=? ORDER BY date_reunion DESC', [id])
                : [];
            // Le sélecteur « Projet lié » n'a de sens que si le créateur possède
            // au moins un projet dans le module Projets.
            try {
                const owner = await pgDb.get(`
                    SELECT 1 FROM projets.projets
                    WHERE created_by_username = ? OR chef_projet_username = ?
                       OR commanditaire_username = ? OR responsable_dsi_username = ?
                    LIMIT 1
                `, [reunion.created_by, reunion.created_by, reunion.created_by, reunion.created_by]);
                reunion.creator_has_projects = !!owner;
            } catch (e) { reunion.creator_has_projects = false; }
            res.json(reunion);
        } catch (error) { res.status(500).json({ error: error.message }); }
    },

    // POST: Envoyer le compte rendu (bloqué si le compte rendu est vide)
    sendCompteRendu: async (req, res) => {
        try {
            const { id } = req.params;
            const built = await buildCompteRendu(id);
            if (!built) return res.status(404).json({ error: 'Réunion non trouvée' });
            if (built.isEmpty) {
                return res.status(400).json({ error: "Le compte rendu est vide : renseignez au moins un déroulé, une décision ou une tâche avant l'envoi." });
            }
            const emails = built.recipients.map(r => r.email);
            ...
            for (const att of attachments) {
                ...
                if (storagePath) {
                    ...
                    if (fs.existsSync(filePath)) {
                        fileAttachments.push({ filename: att.original_name, content: fs.readFileSync(filePath).toString('base64') });
                    }
                }
            }
            ...
            for (const email of emails) {
                try { await sendMailFn(email, built.subject, built.content, fileAttachments); sent++; }
    ...
    // GET: Prévisualisation du compte rendu (contenu + destinataires) avant envoi
    previewCompteRendu: async (req, res) => {
        try {
            const { id } = req.params;
            const built = await buildCompteRendu(id);
            if (!built) return res.status(404).json({ error: 'Réunion non trouvée' });
...
${teamsHtml}`;
    ...
    addParticipant: async (req, res) => {
        try {
            const { id } = req.params;
            const { nom, prenom, email, service, direction, type_presence, statut_presence, ad_username, commentaire, organisme, fonction } = req.body;
            const result = await pgDb.run(
                `INSERT INTO reunion_participants (reunion_id, nom, prenom, email, service, direction, type_presence, statut_presence, ad_username, commentaire, organisme, fonction) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [id, nom, prenom, email, service, direction, type_presence || 'metier', statut_presence || 'present', ad_username, commentaire || null, organisme || null, fonction || null]
            );
    ...
    // PUT: Mettre à jour un participant (notamment son statut de présence)
    updateParticipant: async (req, res) => {
        try {
            const { id } = req.params;
            const existing = await pgDb.get('SELECT * FROM reunion_participants WHERE id=?', [id]);
            if (!existing) return res.status(404).json({ error: 'Participant non trouvé' });
            const { nom, prenom, email, service, direction, type_presence, statut_presence, ad_username, commentaire, organisme, fonction } = req.body;
            await pgDb.run(
                `UPDATE reunion_participants SET nom=?, prenom=?, email=?, service=?, direction=?, type_presence=?, statut_presence=?, ad_username=?, commentaire=?, organisme=?, fonction=? WHERE id=?`,
                [
                    nom !== undefined ? nom : existing.nom,
                    prenom !== undefined ? prenom : existing.prenom,
                    email !== undefined ? email : existing.email,
                    service !== undefined ? service : existing.service,
                    direction !== undefined ? direction : existing.direction,
                    type_presence !== undefined ? type_presence : existing.type_presence,
                    statut_presence !== undefined ? statut_presence : existing.statut_presence,
                    ad_username !== undefined ? ad_username : existing.ad_username,
                    commentaire !== undefined ? commentaire : existing.commentaire,
                    organisme !== undefined ? organisme : existing.organisme,
                    fonction !== undefined ? fonction : existing.fonction,
                    id,
                ]
            );
            const participant = await pgDb.get('SELECT * FROM reunion_participants WHERE id=?', [id]);
