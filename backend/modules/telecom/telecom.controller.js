...
// On ne propose que les factures de l'année courante (+ décembre N-1). Le n° de facture et la date
...
async function findBudgetInvoicesForOperator(operatorId, tierCode) {
    ...
    const candidateSql = `
        SELECT * FROM (
            SELECT COALESCE(
                       substring(f."FACTURE_LIBELLE1" from 'N°([^ ]+)'),
                       substring(f."FACTURE_LIBELLE1" from '([A-Z]\\d{4}VTF\\d{4,5})'),
                       NULLIF(TRIM(f."FACTURE_FACTIERS"), '')
                   ) as invoice_number,
...
module.exports = {
    ...
    createOperator: async (req, res) => {
        const { name, logo_url, tier_id, tier_code } = req.body;
        try {
            const result = await pgDb.run(
                'INSERT INTO hub_telecom.operators (name, logo_url, tier_id, tier_code) VALUES (?, ?, ?, ?)',
                [name, logo_url, tier_id || null, tier_code || null]
            );
    ...
    updateOperator: async (req, res) => {
        ...
        try {
            ...
            if (tier_code !== undefined) { sets.push('tier_code = ?'); params.push(tier_code || null); }
