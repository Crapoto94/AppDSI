...
module.exports = {
    ...
    async create({ name, base_document_id, fields, is_default, created_by }) {
        if (is_default) await pgDb.run(`UPDATE hub_stocks.bl_templates SET is_default = FALSE`);
        const r = await pgDb.run(
            `INSERT INTO hub_stocks.bl_templates (name, base_document_id, fields, is_default, created_by)
             VALUES ($1, $2, $3::jsonb, $4, $5)`,
            [name, base_document_id || null, JSON.stringify(fields || []), !!is_default, created_by || null]
        );
        return r.lastID;
    },
    async update(id, { name, fields, is_default }) {
