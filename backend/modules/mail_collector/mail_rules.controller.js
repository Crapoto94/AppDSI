...
module.exports = {
  ...
  create: async (req, res) => {
    try {
      const { name, type, keywords, priority, is_active } = req.body;
      ...
      const result = await pgDb.run(
        'INSERT INTO hub_tickets.mail_rules (name, type, keywords, priority, is_active) VALUES (?, ?, ?, ?, ?)',
        [name, type, keywords, priority || 100, is_active !== false]
      );
